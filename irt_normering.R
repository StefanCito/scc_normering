# Dit script wordt gebruikt om de IRT-normering uit te voeren voor een of meerdere doorstroomtoetsen

# De output van dit script bestaat per doorstroomtoets uit:
...
# Daarnaast worden er twee bestanden weggeschreven met overzichtstabellen met alle doorstroomtoetsen met behaalde referentieniveaus en
# behaalde toetsadviezen, met bestandsnamen overzicht_referentieniveaus.csv en overzicht_toetsadviezen.csv
# Tot slot is er een Excelbestand overzicht_correlatiematrices.xlsx met correlatiematrices van alle onderdelen voor alle doorstroomtoetsen
...
# De volgende databestanden worden verwacht per toetstoets:
# - Leerlingdata (TOETS_leerlingen.csv)
# - Scoredata per onderdeel (TOETS_ONDERDEEL.csv)
data_folder = 'dummy_data'

# Toetsen waar we rekening houden met verschillende MML-populaties
populatie_toets = NULL

# Bestand met bestaande gezamenlijk ankerparameters (uitvoer van kalibreer_anker.R)
anker_file = 'dummy_resultaten/ankerparameters_2026.csv'

# Bestand met ingrepen in ankeritems (intern of gezamenlijk), bevat kolommen jaar, toets, item_id, onderdeel en actie (los of uit)
ingrepen_file = 'dif_ingrepen.csv'

# Bestand met cesuren en onderdeelgewichten, bevat tabbladen ref_cesuren, toetsadvies_cesuren en onderdeelgewichten
normeringsgegevens_file = 'normeringsgegevens_2026.xlsx'
...
# Lees ingrepen-bestand in
ingrepen = read.csv2(ingrepen_file)

# Check op ingrepen
if (!all(ingrepen$actie %in% c('los', 'uit'))) {
  warning(paste0('Niet alle acties in het bestand met ingrepen zijn gelijk aan "los" of "uit", deze worden genegeerd.'))
}
...
for (leerling_file in leerling_files) {
  ...
  toets = gsub('_leerlingen.csv', '', leerling_file, fixed = TRUE)

  message(paste0('Bezig met toets ', toets, '...'))
  ...
  # Als er geen MML-populaties zijn schrijven we hier de toetsnaam als placeholder
  if (!toets %in% populatie_toets) {
    leerlingen$populatie = toets
  } 
  leerlingen = leerlingen[, c('person_id', 'schooltype', 'populatie', 'schooladvies')]

  # Lees noscore-items in indien nodig
  noscore_items = NULL
  if (length(noscore_files[grepl(toets, noscore_files)]) > 0) {
    noscore_items = readLines(file.path(data_folder, noscore_files[grepl(toets, noscore_files)]))
  }

  # Sheet voor correlaties
  openxlsx::addWorksheet(wb, toets)
  wb_rows = 1

  all_abilities = NULL
  all_removed_items = NULL
  toets_parameters = list()
  toets_populations = list()
  # Loop over ieder onderdeel om de kalibratie uit te voeren
  for (onderdeel_file in onderdeel_files[grepl(toets, onderdeel_files)]) {

    onderdeel = gsub(paste0(toets, '_|\\.csv'), '', onderdeel_file)
    ...
    # Verwijder noscore-items
    if (!is.null(noscore_items)) {
      score_data = score_data[!score_data[, 'item_id'] %in% noscore_items, ]
    }

    # Verwijder uitgeschakelde ankeritems
    score_data = score_data[!score_data[, 'item_id'] %in% ingrepen[ingrepen[, 'onderdeel'] == onderdeel & ingrepen[, 'toets'] %in% c(toets, 'alle') & ingrepen[, 'jaar'] %in% c(jaar, 'alle') & ingrepen[, 'actie'] == 'uit', 'item_id'], ]

    # Koppel eventueel ook ankeritems los
    for (item_id in ingrepen[ingrepen[, 'onderdeel'] == onderdeel & ingrepen[, 'toets'] %in% c(toets, 'alle') & ingrepen[, 'jaar'] %in% c(jaar, 'alle') & ingrepen[, 'actie'] == 'los', 'item_id']) {
      score_data[score_data[, 'item_id'] == item_id, 'item_id'] = paste0('x', item_id, '_', toets)
    }

    # Voeg persoonsparameters toe en maak kalibratiedata
    score_data = dplyr::inner_join(score_data, leerlingen, by = 'person_id')
    cal_data = score_data[score_data[, 'schooltype'] %in% c(1, 2, 3), ]
    cal_data$populatie = paste0(cal_data$populatie, '_', cal_data$schooltype)
    ...
    toets_populations[[length(toets_populations) + 1]] = populations

    # Itemparameters
    parameters = as.data.frame(coef(cal))
    parameters$onderdeel = onderdeel
    toets_parameters[[length(toets_parameters) + 1]] = parameters[, colnames(parameters) != 'item_score']
  ...
  for (onderdeel in unique(ref_cesuren$onderdeel)) {
    for (refniveau in unique(ref_cesuren[ref_cesuren[, 'onderdeel'] == onderdeel, 'niveau'])) {
      ...
      toets_ref_behaald = rbind(toets_ref_behaald, data.frame('toets' = toets, 'onderdeel' = onderdeel, 'niveau' = refniveau, 'perc_behaald' = perc_behaald))
    }
  }  
  ...
  if (any(cor_matrix < 0.4)) {
    warning(paste0('Er waren correlaties tussen onderdelen lager dan 0.4 bij ', toets, '.'))
  } 
  openxlsx::writeData(wb, toets, as.data.frame(cor_matrix), borders = 'none', rowNames = TRUE)
  ...
  toets_toetsadviezen = NULL
  toets_cesuren = toetsadvies_cesuren
  ...
  toetsadviezen_behaald$toets = toets

  toets_toetsadviezen = rbind(toets_toetsadviezen, toetsadviezen_behaald[, colnames(toetsadviezen_behaald) != 'n'])
  toets_toetsadviezen = toets_toetsadviezen[order(match(toets_toetsadviezen$toetsadvies, toetsadviezen)), c('toets', 'toetsadvies', 'perc_behaald')]

  # Vanaf hier genereren we output

  # Schrijf verwijderde items weg
  if (!is.null(all_removed_items)) {
    write.csv2(dplyr::mutate_if(all_removed_items[, colnames(all_removed_items) != 'item_score'], is.numeric, round, digits = 5), paste0(output_folder, '/', toets, '_uitgeschakeld.csv'), row.names = FALSE, quote = FALSE)
  } else {
    file.create(paste0(output_folder, '/', toets, '_uitgeschakeld.csv')) # Schrijf leeg bestand
  }

  # Schrijf itemparameters weg
  toets_parameters = dplyr::bind_rows(toets_parameters)
  write.csv2(dplyr::mutate_if(toets_parameters, is.numeric, round, digits = 5), paste0(output_folder, '/', toets, '_itemparameters.csv'), row.names = FALSE, quote = FALSE, na = '')

  # Schrijf populatieparameters weg
  toets_populations = dplyr::bind_rows(toets_populations)
  toets_populations = toets_populations[, c('onderdeel', colnames(toets_populations)[colnames(toets_populations) != 'onderdeel'])]
  write.csv2(dplyr::mutate_if(toets_populations, is.numeric, round, digits = 5), paste0(output_folder, '/', toets, '_populatieparameters.csv'), row.names = FALSE, quote = FALSE, na = '')
  ...
  write.csv2(dplyr::mutate_if(all_abilities, is.numeric, round, digits = 5), paste0(output_folder, '/', toets, '_vaardigheden.csv'), row.names = FALSE, quote = FALSE, na = '')

  # Schrijf ook behaalde referentieniveaus en toetsasviezen weg
  write.csv2(toets_ref_behaald[, colnames(toets_ref_behaald) != 'toets'], paste0(output_folder, '/', toets, '_referentieniveaus.csv'), row.names = FALSE, quote = FALSE)
  write.csv2(toets_toetsadviezen[, colnames(toets_toetsadviezen) != 'toets'], paste0(output_folder, '/', toets, '_toetsadviezen.csv'), row.names = FALSE, quote = FALSE, na = '')

  summary_refs = rbind(summary_refs, toets_ref_behaald)
  summary_toetsadviezen = rbind(summary_toetsadviezen, toets_toetsadviezen)
}

# Schrijf samenvattingsbestanden weg
summary_refs = reshape(summary_refs, idvar = c('onderdeel', 'niveau'), timevar = 'toets', direction = 'wide', sep = '_')
summary_toetsadviezen = reshape(summary_toetsadviezen, idvar = c('toetsadvies'), timevar = 'toets', direction = 'wide', sep = '_')
